...
**po-agent** — AI agent for purchase order intake & fulfillment. Email arrives via webhook → classify if valid PO → extract PDF data via OCR + LLM → validate → track in Google Sheets → send confirmation email.

**Current state**: Phase 2 complete. 203 tests passing, all eval thresholds met. Moving to Phase 3 (Composio integration, FastAPI webhook).
...
└── integration/       # Real LLM/OCR calls, marked @pytest.mark.integration
```
...
`@opik.track` on node `__call__` methods and service methods. Root trace created at entrypoint (eval runner or webhook handler). Opik auto-nests spans.
...
OPIK_API_KEY=...              # Required for Opik dashboard
OPIK_WORKSPACE=...            # Opik workspace name
OPIK_PROJECT_NAME=po-workflow # Opik project name
COMPOSIO_API_KEY=...          # Phase 3 (not yet needed)
```

## Current Eval Scores (Phase 2, happy_path)
...
- **Phase 3** (next): Composio integration (real Gmail/Sheets), FastAPI webhook endpoint.
- **Phase 4** (future): Docker, README with architecture/tradeoffs, eval report.
